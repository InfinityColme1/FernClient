...
class RecognitionEngine {
  ...
  Future<SidecarClient> _connect() async {
    ...
    final process = await SidecarProcess.start(
      ...
      [paths.sidecarScript],
      environment: {
        // Que ultralytics no escriba su configuración en la carpeta personal
        // del usuario: todo lo suyo se queda dentro de la de reconocimiento.
    ...
    final fresh = SidecarClient(process);
    _client = fresh;
  ...
  void _touch() {
    _idleTimer?.cancel();
    _idleTimer = Timer(sidecarIdleTimeout, () => unawaited(_stopSidecar()));
  }
